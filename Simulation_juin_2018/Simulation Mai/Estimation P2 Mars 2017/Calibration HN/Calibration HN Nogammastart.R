...
gsqrt <- function(para_h,ret,h,rt)
{
  ...
  if (drapeau==0){
    resultat= a0 +b1*h+a1*(((ret-rt-lamdastar0*(h))/((h)^(1/2))) - gamastar*((h)^(1/2)))^2
  }else{
    resultat=NA
  }
  return(resultat)
...
FC_Q<-function(x,para_h,Data.ret ,Data.N){
  ...
  for (i in 1:Z){
    ...
    for (j in 1:steps){
      ...
      B_Q= u*lamdastar0+b1*B_Q+(((u^2)/2)+a1*B_Q*(gamastar^2)-2*u*gamastar*a1*B_Q)/(1-2*a1*B_Q)
    }
...
RMSEopti <- function(para_h,Data.ret,Data.N)
{  
  C=Data.N$C       ####  Call price
  P<-Price_fft(para_h=para_h,Data.ret=Data.ret, Data.N=Data.N)
...
{  
  ...
  rmse<-sqrt((mean(error)))
  return(list(rmse=rmse,P=P,Er=error)) 
}
