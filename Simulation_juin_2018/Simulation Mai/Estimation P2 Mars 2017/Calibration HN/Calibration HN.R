...
gsqrt <- function(para_h,ret,h,rt)
{
  ...
  z1=1 - b1 - a1*(gamastar)^2 
  
  
  drapeau=0
  if (a0<=0){drapeau=1}
  if (a1<=0){drapeau=1}
  if (gamastar<=0){drapeau=1}
  if (b1<=0){drapeau=1}
  ...
  if (drapeau==0){
    resultat= a0 +b1*h+a1*(((ret-rt-lamda0*(h))/((h)^(1/2))) - gamastar*((h)^(1/2)))^2
  }else{
    resultat=NA
  }
  return(resultat)
...
####################################################
###### A- Under the Risk neutral probability      ##
####################################################
...
FC_Q<-function(x,para_h,Data.ret ,Data.N){
  ...
  for (i in 1:Z){
    ...
    for (j in 1:steps){
      A_Q= A_Q+ r[i]*u + a0*B_Q-(1/2)*log(1-2*a1*B_Q)
      B_Q= u*lamda0+b1*B_Q+(((u^2)/2)+a1*B_Q*(gamastar^2)-2*u*gamastar*a1*B_Q)/(1-2*a1*B_Q)
    }
