...
ht<-function(para_h,Data.ret){
  ...
  h_star[1]=(a0 )/(1 - b1 - a1- a2/2)                           ####  The first value for h,
  for (i in 2:Z1){
    h_star[i]=a0 +b1*h[i-1] +(a1*(ret[i-1]-rt[i-1]-(-h[i-1]/2)-lamda0)^2)+ (a2*(max(0,-(ret[i-1]-rt[i-1]-(-h[i-1]/2)+lamda0)))^2)
  }
...
FC_Q<-function(x,para_h,Data.ret ,Data.N){
  ...
  # The volatility under the Q probability
  h<-ht(para_h,Data.ret)
  ...
  for (i in 1:Z){
    ...
    for (j in 1:steps){
      ...
      B_Q= b1*B_Q-(1/2)*u+  (1/(1-2*a1*B_Q))*(((u^2)/2) -2*a1*gamastar*B_Q*u+a1*B_Q*(gamastar^2))
    }
