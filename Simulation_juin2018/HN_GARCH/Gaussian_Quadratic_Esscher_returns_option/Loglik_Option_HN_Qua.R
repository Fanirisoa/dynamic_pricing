...
fsqrt <- function(para_h,ret,h,rt)
{
  ...
  h0=(a0star + a1star)/(1 - b1 - a1star*(gamastar)^2)    
  ...
  if (drapeau==0){
    resultat= a0star +b1*h+a1star*(((ret-rt-lamda0star*(h))/(sqrt(h))) - gamastar*(sqrt(h)))^2
  }else{
    resultat=NA
  }
  return(resultat)
...
h<-function(para_h,Data.ret){
  ...
  h_star[1]=(a0star + a1star)/(1 - b1  - a1star*(gamastar)^2 )                     ####  The first value for h,
  for (i in 2:Z1){
...
FC_Q<-function(x,para_h,Data.ret ,Data.N){
  ...
  for (i in 1:Z){
    ...
    for (j in 1:steps){
      A_Q= A_Q+ r[i]*u + a0star*B_Q-(1/2)*log(1-2*a1star*B_Q)
      B_Q= u*lamda0+b1*B_Q+(((u^2)/2)+a1star*B_Q*(gamastar^2)-2*u*gamastar*a1star*B_Q)/(1-2*a1star*B_Q)
    }
...
###########################################################
Heston_likelihood_opti <- function(para_h,Data.ret, Data.N) {
