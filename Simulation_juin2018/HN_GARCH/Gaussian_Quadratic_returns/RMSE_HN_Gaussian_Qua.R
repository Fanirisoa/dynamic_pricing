...
ht<-function(para_h,Data.ret){
  ...
  for (i in 2:Z1){
    h_star[i]=a0star +b1*h_star[i-1]+a1star*(((ret[i-1]-rt[i-1]-lamda0star*(h_star[i-1]))/(sqrt(h_star[i-1]))) - gamastar*(sqrt(h_star[i-1])))^2
  }
...
FC_Q<-function(x,para_h,Data.ret ,Data.N){
  ...
  for (i in 1:Z){
    ...
    for (j in 1:steps){
      ...
      B_Q= b1*B_Q-(1/2)*u+  (1/(1-2*a1star*B_Q))*(((u^2)/2) -2*a1star*gamastar*B_Q*u+a1star*B_Q*(gamastar^2))
    }
