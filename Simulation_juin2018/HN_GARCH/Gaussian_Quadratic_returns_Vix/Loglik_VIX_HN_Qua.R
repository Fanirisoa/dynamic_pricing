...
gsqrt <- function(para_h,ret,h,rt)
{
  ...
  g1= b1 + a1star*(gamastar^2)           ####  The percistence
  h0=(a0star + a1star)/(1 - g1)              ####  The first value for h, Unconditional Variance
  z2=1 - b1 - a1star*(gamastar)^2 
  ...
  if (drapeau==0){
    resultat= a0star +b1*h+a1star*(((ret-rt-lamda0star*(h))/((h)^(1/2))) - gamastar*((h)^(1/2)))^2
  }else{
    resultat=NA
  }
  return(resultat)
