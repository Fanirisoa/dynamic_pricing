...
############################################################
rmse1 <- function(par,Data.ret,Data.contract)
{  
  C=Data.contract$C       ####  Call price
...
start.time <- Sys.time()
rmse1(par=para_h,Data.ret=Data.ret,Data.contract=Data.contract)
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken
...
#### NLS estimation                                       ##
############################################################
para_h<-c(0,0,0,0,0,0,0,0)

start.time <- Sys.time()
NLS_estim=optim(par=para_h,fn=rmse1,Data.ret=Data.ret,Data.contract=Data.contract,method="Nelder-Mead",lower=c(-101,-102,-103,-104,-105,-106,-107,-108), upper=c(101,102,103,104,105,106,107,108))
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken

rmse1 <- function(para_h)
{  
  C=Data.contract$C       ####  Call price
...
nlm(rmse1, para_h <- c(-1,0.001,0,-0.5,0,-0.5,-0.1,-0.2), hessian=TRUE)
