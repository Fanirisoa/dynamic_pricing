...
h_star<-function(para_h,para_h_star,Data.ret){
  ...
  #para_h_star<-c()  set up the parameters of the model neta_star=para_h_star[2]
  Pi=para_h_star[1]
...
MGF_P<-function(u,para_h ,Data.ret,S,T,r1){
  # ret                    ####  returns,  
  
  steps<-round(T*250,0)  #### Time to maturity expressed in terms of years in terms of days
  #S                     #### Prix du sous-jacent: Data.contract$S
  #r1 <- r1               #### Taux d'interet sans risque Data.contract$r
  ...
  for (i in 1:steps){
    A= A+ r1*u +w*B-(1/2)*log(1-2*a*(neta^4)*B)
    B= b*B+u*nu+ (neta^(-2))*( 1- sqrt((1-2*a*(neta^4)*B)*(1-2*c*B-2*u*neta))) 
  }
  
  MGF_P = exp(log(S)*u + A + B*h )
  return(MGF_P)
  
}

para_h<-c(1,-0.001,0,0.5,0,0)
para_h_star<-0.1
M = mapply(function(S,T,r1) MGF_P(u=0.002, para_h=para_h, Data.ret=Data.ret,S,T,r1) ,S=Data.contract$S,T=Data.contract$T,r1=Data.contract$r)
M
...
FC_P<-function(u,para_h ,Data.ret ,S ,T,r1){
  # Complex number : 
  u_Complex=1i*u 
  
  FC_P= MGF_P(u_Complex,para_h ,Data.ret ,S ,T,r1)
  
  return(FC_P)
  
}

M1<-mapply(function(S,T,r1) FC_P(u=0.002,para_h=para_h, Data.ret=Data.ret,S,T,r1) ,S=Data.contract$S,T=Data.contract$T,r1=Data.contract$r)


FC_P2<-function(u,para_h ,Data.ret ,Data.contract){
  # Complex number : 
  u_Complex=1i*u 
  
  FC_P= mapply(function(S,T,r1) MGF_P(u_Complex, para_h=para_h, Data.ret=Data.ret,S,T,r1) ,S=Data.contract$S,T=Data.contract$T,r1=Data.contract$r)
  
  return(FC_P)
  
}

M2<-FC_P2(u=0.002,para_h=para_h, Data.ret=Data.ret,Data.contract=Data.contract)
...
###### The moment generating function at tau      ##
####################################################
MGF_Q<-function(u,para_h,para_h_star,Data.ret,S ,T,r1){
  # ret                    ####  returns,  
  
  steps<-round(T*250,0)  #### Time to maturity expressed in terms of years in terms of days
  #S                     #### Prix du sous-jacent: Data.contract$S
  r1 <- r1/250               #### Taux d'interet sans risque Data.contract$r
  ...
  for (i in 1:steps){
    A_Q= A_Q+ r1*u + w*Pi*B_Q-(1/2)*log(1-2*(para_h[6]/Pi)*neta*(neta_star^3)*B_Q)
    B_Q= b*B_Q+u*(nu/Pi)+ ((neta_star)^(-2))*( 1- sqrt((1-2*(a/Pi)*neta*(neta_star^3)*B_Q)*(1-2*c*Pi*(neta_star/neta)*B_Q-2*u*neta_star))) 
    
  }
  
  MGF_Q = exp(log(S)*u + A_Q + B_Q*h_star )
  
  return(MGF_Q)
  
}

para_h<-c(-1,0.001,0,-0.5,0,-0.5)
para_h_star<-c(-0.1,-0.2)
mapply(function(S,T,r1) MGF_Q(u=0.08, para_h=para_h,para_h_star=para_h_star, Data.ret=Data.ret,S,T,r1) ,S=Data.contract$S,T=Data.contract$T,r1=Data.contract$r)
...
FC_Q2<-function(u, para_h ,para_h_star,Data.ret ,S ,T,r1){
  # Complex number : 
  u_Complex=1i*u 
  
  FC_Q= MGF_Q(u_Complex, para_h ,para_h_star,Data.ret ,S ,T,r1)
  
  return(FC_Q)
  
}

W1<-mapply(function(S,T,r1) FC_Q2(u=0.002,para_h=para_h,para_h_star=para_h_star, Data.ret=Data.ret,S,T,r1) ,S=Data.contract$S,T=Data.contract$T,r1=Data.contract$r)

FC_Q<-function(u, para_h ,para_h_star,Data.ret ,Data.contract){
  # Complex number : 
  u_Complex=1i*u 
  
  FC_Q=  mapply(function(S,T,r1) MGF_Q(u_Complex, para_h=para_h,para_h_star=para_h_star, Data.ret=Data.ret,S,T,r1) ,S=Data.contract$S,T=Data.contract$T,r1=Data.contract$r)
  
  return(FC_Q)
  
}
W<-FC_Q(u=0.002, para_h ,para_h_star,Data.ret ,Data.contract)
W

W1-W

start.time <- Sys.time()
FC_Q(u=0.002, para_h ,para_h_star,Data.ret ,Data.contract)
end.time <- Sys.time()
...
Price_fft<-function(para_h,para_h_star,Data.ret,Data.contract){
  ...
  for (i in 1:N){
    ...
    w_FC=w-(alpha+1)*1i
    phi= FC_Q(w_FC, para_h ,para_h_star,Data.ret ,Data.contract)
    phi=phi*exp(-r1*(T))
...
start.time <- Sys.time()
Price_fft(para_h=para_h,para_h_star=para_h_star,Data.ret=Data.ret,Data.contract=Data.contract)
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken
