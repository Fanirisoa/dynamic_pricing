...
h<-function(para_h,Data.ret){
  ...
  h_star[1]=(w + a*(neta^4))/(1 - a*(neta^2) - b - (c*(neta^(-2))))   ####  The first value for h,
  for (i in 2:Z1){
    h_star[i]=w+b*h_star[i-1]+ c*(neta^(-1))*(ret[i-1]-rt[i-1]-(nu*h_star[i-1]))+((a*neta*(h_star[i-1])^2)/(ret[i-1]-rt[i-1]-(nu*h_star[i-1])))
  }
...
####################################################
Fun<-function(para_h,x,r,T,h,S,A_Q ,B_Q,ret,rt ){
  ...
  if (is.na(z)==TRUE){drapeau=1}else{
    if (z<0){drapeau=1}
    if (abs(z)==Inf){drapeau=1}
    if (1/abs(z)==Inf){drapeau=1}
  }
...
####################################################
FC_Q<-function(x,para_h,Data.ret ,Data.N){
  ...
  for (i in 1:Z){
    ...
    for (j in 1:steps){
      A_Q= A_Q+ r[i]*u + w*B_Q-(1/2)*log(1-2*a*(neta^4)*B_Q)
      B_Q= b*B_Q+u*nu+ (1/neta^2)*(1-sqrt((1-2*a*(neta^4)*B_Q)*( 1- 2*c*B_Q - 2*u*neta)))
    }
    FC_Q[i]= Fun(para_h,x,r[i],T[i],h[Per[i]+1],S[i],A_Q ,B_Q,ret[Per[i]+1],rt[Per[i]+1])
  }
  return(FC_Q)
}
