...
Sim<-function(para_h,para_distribution,ht){
  ...
  mt =  -(ht/2) # lamda0*sqrt(ht)-(ht/2)      lamda0*ht 
  
  # gama Constrainte GH distribution
  gama_d=(alpha^2-beta^2)^(1/2)
  
  ## Normalisation :
  sigma_z = (delta*(alpha^2))/((gama_d)^3)
  
  mu_z= mu - ((delta*beta)/(gama_d))
  ...
  gamma_1=((alpha_1)^2-(beta_1)^2)^(1/2)
  
  
  # first part of theta
  A= ((alpha_1*mt+ sqrt(delta_1*ht)* beta_1*  gamma_1)^2)/(ht*delta*( gamma_1^(3)))
  
  # second part of theta  
  B = ((4*(alpha_1^4)*(delta_1^2))/(ht*delta_1*(gamma_1^(3)) + (alpha_1*mt+sqrt(delta_1*ht)* beta_1*gamma_1)^2))-1
  
  # value of theta   
  theta =  -1/2 - ((alpha_1* beta_1*sqrt(delta_1))/(sqrt(ht)*(gamma_1^(3/2)))) -(1/2)*((A*B)^(1/2))
  
  # change in parameter under RN distribution
  beta0=beta_1 + sqrt(ht)*theta
  
  result=rgh(1,alpha_1,beta0,delta_1,mu_1,-1/2)[1]
...
Matrice_ret<-function(x){  
  ...
  h0= (a0 + a1)/(1 - b1 - a1*(gama)^2 )  ###    (a0 + a1)/(1 - g1)              ####  The first value for h, Unconditional Variance
  ...
  for(j in 1:N)  
  {
    ...
    Inv[1]= 0                              ####  innovation starting value  Sim(para_h1,para_distribution1,ht[1]) 
