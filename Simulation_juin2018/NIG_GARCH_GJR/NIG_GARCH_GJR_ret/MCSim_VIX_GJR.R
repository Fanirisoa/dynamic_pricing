...
Seuil<-function(para_h,para_distribution,ht){
  ...
  alpha=para_distribution1[1];  beta=para_distribution1[2];  
...
fun <- function (x)  ((4*(alpha^4)*(delta^2))/(x*delta*(gamma^(3)) + (alpha*lamda0*x+sqrt(delta*x)* beta*gamma)^2)) -1
...
Sim<-function(para_h,para_distribution,ht){
  ...
  mt =  lamda0*sqrt(ht)-(ht/2)   # lamda0*ht
  
  # gama Constrainte GH distribution
  gama=(alpha^2-beta^2)^(1/2)
