...
MGF_P<-function(u,para_h ,Data.ret ,S ,T,r1){
  ...
  for (i in 2:steps){
    A= A+ r1*u + para_h[1]*B-(1/2)*log(1-2*para_h[6]*(para_h[4]^4)*B)
    B= para_h[2]*B+u*para_h[5]+ (para_h[4]^(-2))*( 1- sqrt((1-2*para_h[6]*(para_h[4]^4)*B[i-1])*(1-2*para_h[3]*B-2*u*para_h[4]))) 
  }
  
  MGF_P = exp(log(S)*u + A + B*h1 )
...
para_h<-c(-1.5,-1,0.5,0.4,-0.5,0)

mapply(function(S,T,r1) MGF_P(u=-1, para_h=para_h, Data.ret=Data.ret,S,T,r1),S=Data.contract$S,T=Data.contract$T,r1=Data.contract$r)
