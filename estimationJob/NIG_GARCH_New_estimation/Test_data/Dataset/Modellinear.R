...
Serie.data <- setNames(lapply(1:4, function(s) Data.function(s)),paste0("d_", 1:4))  
...
CF_P<-function(u,para_h ,Data.contract){
  ...
  for (i in 1:Z){
    ...
    for (j in 1:steps){
      A= A+ r1[i]*u1 +w*B-(1/2)*log(1-2*a*(neta^4)*B)
      B= b*B+u1*nu+ (neta^(-2))*( 1- sqrt((1-2*a*(neta^4)*B)*(1-2*c*B-2*u1*neta))) 
    }
    
    CF_P[i]= exp(log(S[i])*u1 + A + B*h )
  }
  return(CF_P)
...
## M1 = CF_P(u=0.002, para_h=para_h, Data.contract=Data.contract)
## M1
...
CF_Q<-function(u,para_h,Data.contract){
  T=Data.contract$T       ####  Time to maturity expressed in terms of years in terms of days
  S=Data.contract$S       ####  Prix du sous-jacent: Data.contract$S
  K=Data.contract$K       ####  Strike  Prix d'exercice: data$strike
  r=Data.contract$r      ####  Interest rate Data.contract$r  
  ...
  for (i in 1:Z){
    ...
    steps<-round(T[i]*250,0)  #### Time to maturity expressed in terms of years in terms of days
    for (j in 1:steps){
      A_Q= A_Q+ r1[i]*u1 + w*Pi*B_Q-(1/2)*log(1-2*(a/Pi)*neta*(neta_star^3)*B_Q)
      B_Q= b*B_Q+u1*(nu/Pi)+ ((neta_star)^(-2))*( 1- sqrt((1-2*(a/Pi)*neta*(neta_star^3)*B_Q)*(1-2*c*Pi*(neta_star/neta)*B_Q-2*u1*neta_star))) 
    }
    CF_Q[i]= exp(log(S[i])*u1 + A_Q + B_Q*h_star )
  }
  return(CF_Q)
}
...
## para_h<-c(1,-0.001,0,0.5,0,0,-0.1,0.2)
## start.time <- Sys.time()
## W1=CF_Q(u=0.08, para_h=para_h,Data.contract=Data.contract)
## end.time <- Sys.time()
...
Price_fft<-function(para_h,Data.contract){
  ...
  for (i in 1:N){
    ...
    w_FC=w-(alpha+1)*1i
    phi= CF_Q(w_FC, para_h,Data.contract)
    phi=phi*exp(-r1*(T))
...
rmse2 <- function(par,Data.contract)
{  
  C=Data.contract$C       ####  Call price
  P<-Price_fft(para_h=par,Data.ret=Data.ret,Data.contract=Data.contract)
  V<-vega_call(Data.contract=Data.contract,type="C")
  error <- rep(NA, length(C))
  for (i in 1:length(C)){
    error[i] = ((P[i]  -  C[i])/V[i])^2
  }
  rmse2<-100*sqrt(mean(error))
  return(rmse2)
}
start.time <- Sys.time()
rmse2(par=para_h,Data.contract=Data.contract)
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken
