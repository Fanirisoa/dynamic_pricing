...
###########################################
setwd("G:/Historique/Sim_nov_2017_paper_2/Dataset chorro") 
...
#Data.N=Data.N1[-c(85,115,146,198,203,356,376,404,440,462,488,519,568,618,665,701,756,780,861,1049,1056,1073,1102,1153,1177,1297),]

##########################################
### Code to extracte the base_SJ   #######
##########################################
 setwd("G:/Historique/Sim_nov_2017_paper_2/Dataset chorro/Dataset") 
...
#####################################################
save(Data.ret,Data.N,Data.returns, file="DataPrice2009.Rdata")

########################################################################################
###     Plot prices, daily log returns & running mean of log returns since 1990  #######
########################################################################################

df <- tryCatch(Data.returns, error = function(e) NA)
...
plot(subset(df, date >= start.date)[ , c("date", "st")], type="l",
     main="(a) S&P 500 Index since 1999.", xlab="Dates", ylab="Index value", col="1")

plot(subset(df, date >= start.date)[ , c("date", "ret")], type="l",
     main="(b) S&P 500 Log-Returns since 1999.", xlab="Dates", ylab="Log-Returns", col="1")

########################################################################
###     Plot VIX index , daily log VIX returns  since 1990       #######
########################################################################

start.date <- "1999-01-07"
plot(subset(df, date >= start.date)[ , c("date", "vix")], type="l",
     main="(c) S&P 500 VIX-Index since 1999.", xlab="Dates", ylab="Index value", col="1")

plot(subset(df, date >= start.date)[ , c("date", "vixret")], type="l",
     main="(d) S&P 500 Log-Returns-VIX since 1999.", xlab="Dates", ylab="Log-Returns", col="1")


########################################################################
###           Descriptive Statistics VIX index  since 1990       #######
########################################################################
retstat<-data.frame(A1=df$st,A2=df$ret,A3=df$vix, A4=df$vixret)

statsMat = rbind(apply(retstat, 2, length),apply(retstat, 2, min),apply(retstat, 2, max), 
                 + apply(retstat, 2, mean),apply(retstat, 2, var), apply(retstat, 2, sd),
                 + apply(retstat, 2, skewness), apply(retstat, 2, kurtosis))
rownames(statsMat) = c("number","Min","Max","Mean", "Variance", "Std Dev","Skewness", "Excess Kurtosis")
