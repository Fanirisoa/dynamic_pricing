...
Sim<-function(para_h,para_distribution,ht){
  ...
  # first part of theta
  A= ((alpha*mt+ sqrt(delta*ht)* beta*gamma)^2)/(ht*delta*( gamma^(3)))
  
  # second part of theta  
  B = ((4*(alpha^4)*(delta^2))/(ht*delta*(gamma^(3)) + (alpha*mt+sqrt(delta*ht)* beta*gamma)^2))-1
  
  # value of theta   
  theta =  -1/2 - ((alpha* beta*sqrt(delta))/(sqrt(ht)*(gamma^(3/2)))) -(1/2)*((A*B)^(1/2))
